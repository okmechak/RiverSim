...
BOOST_PYTHON_MODULE(riversim)
{
    ...
    //tree
    class_<Branch, bases<t_PointList>>("Branch", "Holds all functionality that you need to work with single branch.")
        .def(init<River::Point&, double>(args("source_point", "angle")))
        .def("addAbsolutePoint", static_cast< Branch& (Branch::*)(const River::Point&)>( &Branch::AddAbsolutePoint), return_internal_reference<>(), "Adds point p to branch with absolute coords.")
        .def("addAbsolutePoint", static_cast< Branch& (Branch::*)(const Polar&)>( &Branch::AddAbsolutePoint), return_internal_reference<>(), "Adds polar p coords to branch with absolute angle, but position is relative to tip.")
        .def("addPoint", static_cast< Branch& (Branch::*)(const River::Point&)>( &Branch::AddPoint), return_internal_reference<>(), "Adds point p to branch in tip relative coord system.")
        .def("addPoint", static_cast< Branch& (Branch::*)(const Polar&)>( &Branch::AddPoint), return_internal_reference<>(), "Adds polar p to branch in tip relative coord and angle system.")
    ...
    class_<Tree, bases<t_Tree> >("Tree", "Combines branches into tree like structure.", init<>())
        .def(init<Tree&>())
        .def(self == self)
        .def("initialize", &Tree::Initialize, "Initialize tree with source points vector and source angle vector.")
        .def("addBranch", &Tree::AddBranch, "Adds new branch with id (id should be unique).")
        .def("addSubBranches", &Tree::AddSubBranches, "Returns root(or source) branch of branch (if there is no such - throw exception).")
        .def("deleteBranch", &Tree::DeleteBranch)
        .def("deleteSubBranches", &Tree::DeleteSubBranches)
        .def("clear", &Tree::Clear, "Clear whole data from tree.")
        ...
        .def("zeroLenghtTipBranchesIds", &Tree::zero_lenght_tip_branches_ids)
        .def("tipPoints", &Tree::TipPoints)
        .def("tipIdsAndPoints", &Tree::TipIdsAndPoints)
        .def("isSourceBranch", &Tree::IsSourceBranch)
        .def("maximalTipCurvatureDistance", &Tree::maximal_tip_curvature_distance)
        .def("flattenTipCurvature", &Tree::flatten_tip_curvature)
        .def("removeTipPoints", &Tree::remove_tip_points)
    ...
    class_<Model>("Model")
        ...
        .def_readwrite("tree", &Model::tree, "Tree data structure.")
