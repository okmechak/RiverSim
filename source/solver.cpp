...
namespace River
{
    ...
    void Solver::static_refine_grid(const double integration_radius, const vector<Point>& tips_points)
    {
    ...
    vector<double> Solver::integrate(const IntegrationParams& integration_params, const Point& point, const double angle)
    {   
        ...
        for(auto dof_cell: dof_handler.active_cell_iterators())
        {
            ...
            if(dist <= integration_params.integration_radius)
            {
                fe_values.reinit (dof_cell);
                fe_values.get_function_values(solution, values);
                auto weight_func_value = integration_params.WeightFunction(dist);
                ...
                for(unsigned param_index = 0; param_index < series_params.size(); ++param_index)
                {
                    ...
                    auto base_vector_value = integration_params.BaseVectorFinal(param_index + 1, angle, dx, dy);
    ...
    void Solver::run(const BoundaryConditions& boundary_conditions)
    {
