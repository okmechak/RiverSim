...
BOOST_AUTO_TEST_CASE( empty_or_edge_condition_test, 
    *utf::tolerance(eps))
{
    Triangle triangle;

    t_PointList empty_holes;
    Boundary empty_boundary;
    BOOST_CHECK_NO_THROW(triangle.generate_quadrangular_mesh(empty_boundary, empty_holes));
}
/*
    ...
    *utf::tolerance(eps))
{
    ...
    mdl.sources = mdl.region.MakeRectangular();
    
    //Rivers object setup
    mdl.tree.Initialize(mdl.region.GetSourcesIdsPointsAndAngles(mdl.sources));
    ...
    for(unsigned long int i = 0; i < 1e4; i ++)
    {
        ...
        Triangle tria;
        tria.Quite =  true;
        tria.generate_quadrangular_mesh(newmesh);
        newmesh.convert();
    }
    ...
    *utf::tolerance(eps))
{
    ...
    triangle.generate_quadrangular_mesh(mesh);
